(function () {
  ...
  function buildBuiltinAnswersPayload() {
    if (!quizData || !quizData.questions) return [];
    saveCurrentAnswer();
    return quizData.questions.map(function (q) {
      var ans = studentAnswers[q.question_id];
      if (q.question_type === "mcq") {
        return { question_id: q.question_id, question_type: "mcq", student_answer: ans != null ? Number(ans) : null };
      }
      if (q.question_type === "fill_blank" && q.blanks && q.blanks.length) {
        var blanks = q.blanks.map(function (b) {
          var obj = typeof ans === "object" && ans ? ans : {};
          return { blank_id: b.blank_id, student_text: (obj[b.blank_id] || "").trim() };
        });
        return { question_id: q.question_id, question_type: "fill_blank", blanks: blanks };
      }
      if (q.question_type === "essay") {
        return { question_id: q.question_id, question_type: "essay", student_answer: (ans || "").trim() };
      }
      return { question_id: q.question_id, question_type: q.question_type || "mcq", student_answer: ans };
    });
  }
  ...
  function submitQuiz() {
    ...
    if (teacherQuizAttemptId != null) {
      ...
      fetch((window.API_BASE || "") + "/api/reading-quiz-answers", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ attempt_id: builtinAttemptId, answers: builtinAnswers, timeTakenSeconds: timeTakenSeconds })
      })
        .then(function (r) { return r.json(); })
        .then(function (saveData) {
          if (!saveData.success) throw new Error("Failed to save answers");
          return fetch((window.API_BASE || "") + "/api/reading-quiz-attempts/" + builtinAttemptId + "/submit", {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ cheating_violations: cheatingViolations, cheating_voided: !!cheatingVoided })
          }).then(function (r) { return r.json(); });
        })
        .then(function (data) {
  ...
  function startBuiltinQuiz(quizId, user) {
          ...
          .then(function (attempts) {
            ...
            if (existing) {
              ...
              return fetch((window.API_BASE || "") + "/api/reading-quiz-attempts/" + existing.attempt_id + "/answers")
                ...
                .then(function (ansList) {
                  ansList.forEach(function (a) {
                    if (a.question_type === "fill_blank" && a.blanks && a.blanks.length) {
                      var obj = {};
                      a.blanks.forEach(function (b) { obj[b.blank_id] = b.student_text || ""; });
                      studentAnswers[a.question_id] = obj;
                    } else {
                      studentAnswers[a.question_id] = a.student_answer;
                    }
                  });
                  return Promise.resolve();
                });
            ...
            return fetch((window.API_BASE || "") + "/api/reading-quiz-attempts", {
              ...
              .then(function (r) { return r.json(); })
              .then(function (data) {
                if (!data.attempt_id) throw new Error("Could not start quiz");
                builtinAttemptId = data.attempt_id;
              });
          })
          .then(function () {
  ...
  function loadBuiltinReview(quizId, user) {
    var selectedClass = null;
    try { selectedClass = JSON.parse(localStorage.getItem("eel_selected_class") || "null"); } catch (_) {}
    var classId = (selectedClass && selectedClass.id != null) ? selectedClass.id : (localStorage.getItem("eel_selected_class_id") || null);
    var attemptParams = "quiz_id=" + quizId + "&student_id=" + encodeURIComponent(user.user_id) + (classId ? "&class_id=" + classId : "");
    fetch((window.API_BASE || "") + "/api/reading-quizzes/" + quizId)
      ...
      .then(function (quiz) {
        return fetch((window.API_BASE || "") + "/api/reading-quiz-attempts?" + attemptParams)
          .then(function (r) { return r.json(); })
          .then(function (attempts) {
            var completed = attempts.filter(function (a) { return a.status === "completed"; })[0];
            if (!completed) throw new Error("You have not taken this quiz yet.");
            return fetch((window.API_BASE || "") + "/api/reading-quiz-attempts/" + completed.attempt_id + "/answers")
              .then(function (r) { if (!r.ok) throw new Error("Could not load answers"); return r.json(); })
              .then(function (answers) {
                ...
                if (container) {
                  container.innerHTML = answers.map(function (a, i) {
                    ...
                    if (a.question_type === "mcq" && q && q.options && q.options.length) {
                      var opt = q.options.find(function (o) { return Number(o.option_id) === parseInt(a.student_answer, 10); });
                      yourAnswerText = opt ? escapeHtml(opt.option_text) : yourAnswerText;
                    }
                    if (a.question_type === "fill_blank" && a.blanks && a.blanks.length) {
                      yourAnswerText = a.blanks.map(function (b, idx) { return "Blank " + (idx + 1) + ": " + escapeHtml(b.student_text || "—"); }).join("; ");
                    }
                    var correctText = a.correct_answer ? escapeHtml(String(a.correct_answer)) : "—";
                    if (a.question_type === "fill_blank" && a.blanks && a.blanks.length) {
